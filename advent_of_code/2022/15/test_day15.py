...
def test_example_input():
    sensors = Sensor.from_file(os.path.join(os.path.dirname(__file__), "example.txt"))
...
def test_find_beacon(fake_tqdm):
    ...
    assert find_beacon(sensors=sensors, search_min=0, search_max=20) == (
        expected_beacon_location,
        expected_tuning_freq,
    )
